...
def get_user(user_id, username="유저"):
    ...
    cur.execute("""
        SELECT user_id, username, points, xp, level, last_attendance
        FROM users
        WHERE user_id = ?
    """, (user_id,))
    row = cur.fetchone()
    ...
    if row is None:
        cur.execute("""
            INSERT INTO users
            (user_id, username, points, xp, level, last_attendance)
            VALUES (?, ?, 0, 0, 1, NULL)
        """, (user_id, username))
        conn.commit()
        ...
        if row is None:
            ...
            if last_attendance == today:
                ...
                cur.execute("""
                    ...
                    WHERE user_id = ?
                """, (new_points, today, username, user_id))
                conn.commit()
                already = False
...
async def level_up(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if changed == 0:
        await update.message.reply_text(
            "❌ 레벨업 처리 중 상태가 변경되었습니다. 다시 시도해주세요."
        )
        return
...
async def buy_lottery(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    async with db_lock:
        ...
        if row is None:
            cur.execute("""
                INSERT INTO users
                (user_id, username, points, xp, level)
                VALUES (?, ?, 0, 0, 1)
            """, (user_id, username))
            points = 0
        else:
...
async def admin_take(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        t = get_user(target_id)
        new_points = max(0, t["points"] - amount)
        update_user(target_id, points=new_points)
...
def create_card_images():
    ...
    rank_font = get_font(34, True)
    suit_font = get_font(34, True)
    center_font = get_font(100, True)
    ...
    for suit_code, (symbol, color) in SUITS.items():
        ...
        for rank in RANKS:
            ...
            draw.text((22, 18), rank, font=rank_font, fill=fill)
            draw.text((22, 55), symbol, font=suit_font, fill=fill)
...
async def send_card(bot, chat_id, card, caption):
    with open(card["file"], "rb") as f:
...
async def play_baccarat(bot, chat_id):
    async with game_lock:
        if not baccarat_game["active"]:
            return
    ...
    player_score = baccarat_score(player)
    banker_score = baccarat_score(banker)

    # 내추럴이면 추가 카드 없음
    if player_score not in (8, 9) and banker_score not in (8, 9):
        ...
        if len(player) == 2:
            ...
            if banker_score <= 2:
                ...
                draw = third in (2, 3, 4, 5, 6, 7)
            elif banker_score == 5:
                draw = third in (4, 5, 6, 7)
            elif banker_score == 6:
                draw = third in (6, 7)
    ...
    # 게임용 포인트 정산
    # Player/Banker: 적중 시 원금의 2배 지급
    # Tie: 적중 시 원금의 9배 지급
    # Player/Banker가 Tie면 원금 반환
    ...
    for user_id, bet in bets.items():
        ...
        if bet_type == result:
            if result == "T":
                ...
                payout = amount * 2
            message = f"🎉 {username}: +{payout:,}P"

        elif result == "T" and bet_type in ("P", "B"):
            payout = amount
            message = f"↩️ {username}: {payout:,}P 반환"
    ...
    if lines:
        ...
        await bot.send_message(
            chat_id=chat_id,
            text="💰 이번 게임 당첨자가 없습니다."
        )


async def start_baccarat(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await play_baccarat(context.bot, chat_id)
...
# ============================================================
# 한국어 명령어용 MessageHandler
# Telegram에서 한국어 슬래시 명령어가 안정적으로 처리되도록
# 별도로 메시지에서도 처리한다.
# ============================================================

async def korean_commands(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not update.message or not update.message.text:
        return
    ...
    # /베팅 같은 인자가 필요한 명령어를 위한 context.args 보정
...
def main():
    ...
    application.add_handler(
        MessageHandler(
            filters.Regex(r"^/(내정보|출석|레벨업|복권|바카라|도움말|지급|차감|베팅)(?:\s+.*)?$"),
            korean_commands
        ),
        group=0
    )
